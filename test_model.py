...
def train_loop(model_name,imgname,epochs,learnrate):
    ...
    for epoch in range(num_epochs):
        ...
        def boxmap(testloder,name,Accuracy):
            ...
            # plt.annotate('Test Accuracy: {:.4f}'.format(Accuracy), xy=(0.5, -0.2), xycoords='axes fraction', ha='center')
            # plt.legend()
...
def vote():
    ...
    with torch.no_grad():
        for inputs, labels in tqdm.tqdm(PublicTestloader):
            for i, model in enumerate(modellist):#同时调出索引和模型
        ...
        # private_test_accuracy = correct / total
        # test_acc_history2.append(private_test_accuracy)
        print(f" Public Test Accuracy: {test_accuracy:.4f}, ")#f"Private Test Loss: {private_test_loss:.4f}, Private Test Accuracy: {private_test_accuracy:.4f}")
        ...
        def boxmap(testloder, name, Accuracy):
            ...
            plt.xlabel('Emotion Class')
            plt.ylabel('Accuracy (%)')
            plt.title(name + 'e Average Accuracy: {:.4f}'.format(Accuracy))
            # plt.annotate('Test Accuracy: {:.4f}'.format(Accuracy), xy=(0.5, -0.2), xycoords='axes fraction', ha='center')
            # plt.legend()
            plt.savefig(name + 'e Accuracy by Class.png')
            plt.savefig(name + 'e Accuracy by Class.pdf')
            plt.show()
...
def vote():
    ...
    test_acc_history2 = []
    with torch.no_grad():
        print(f"加载{modedict}成功，正在投票预测Public")
        for inputs, labels in tqdm.tqdm(PublicTestloader):
            ...
            # 计算平均值
            #ensemble_prediction_avg = torch.stack(predictions).float().mean(dim=0)
            total += labels.size(0)
            correct += ensemble_prediction.eq(labels).sum().item()
            test_accuracy_vote1 = correct / total
            #test_accuracy_avg = ensemble_prediction_avg.eq(labels).sum().item() / total
            test_acc_history1.append(test_accuracy_vote1)
            #test_acc_history2.append(test_accuracy_avg)
        last_accuracy_vote1 = test_acc_history1
        #last_accuracy_avg = test_acc_history2
        print("PublicTest投票准确度:", np.array(last_accuracy_vote1).mean())
    #print("最后一次迭代的平均值准确度:", last_accuracy_avg)

    correct = 0
    total = 0
    test_acc_history2 = []
    with torch.no_grad():
        print(f"加载{modedict}成功，正在投票预测Private")
        for inputs, labels in tqdm.tqdm(PrivateTestloader):
            predictions = []  # 保存每个模型的预测结果
            for i, model in enumerate(modellist):
                model.load_state_dict(torch.load(modedict[i]))
            ...
            total += labels.size(0)
            correct += ensemble_prediction.eq(labels).sum().item()
            test_accuracy_vote2 = correct / total
            # test_accuracy_avg = ensemble_prediction_avg.eq(labels).sum().item() / total
            test_acc_history2.append(test_accuracy_vote2)
            # test_acc_history2.append(test_accuracy_avg)
        last_accuracy_vote2 = test_acc_history2
        # last_accuracy_avg = test_acc_history2
        print("PrivateTest投票准确度:", np.array(last_accuracy_vote2).mean())
    # print("最后一次迭代的平均值准确度:", last_accuracy_avg)
        # 计算各个分类的测试准确率
    def boxmap(testloder, name, Accuracy):
        class_correct = np.zeros(7)
